...
- optional `pane` backend(PTY `/usr/bin/script`) 구현 완료
- optional `remote-root` transport 구현 완료
...
- `dev-plan/implement_20260405_132843.md` 기준 Phase 1~3 완료
  - bounded log reader / CLI `logs`
  - TUI `Logs` viewer / stdout-stderr 전환
  - smoke 문서 동기화
- `dev-plan/implement_20260405_134934.md` 기준 Phase 1~3 완료
  - lost detached worker recovery
  - pending/executing-turn 정합성
  - file collision / cost guardrail / soak failure taxonomy
- `dev-plan/implement_20260405_171538.md` 기준 Phase 1~3 완료
  - poll-mailbox 공통화 / run preset 분리
  - 실패 시나리오 테스트 6종 추가
  - package metadata / engines 정리
- `dev-plan/implement_20260405_181634.md` 기준 Phase 1~3 완료
  - approval context surface 정리
  - permission matcher preset UX 추가
  - 문서 / smoke / test 동기화
- `dev-plan/implement_20260405_185959.md` 기준 Phase 1~3 완료
  - release checklist baseline
  - soak release gate helper
  - 문서 / 운영 절차 릴리즈 동기화
- `dev-plan/implement_20260405_201816.md` 기준 Phase 1~3 완료
  - multi-team overview data contract
  - team picker attention surface
  - 문서 / smoke / regression sync
- `dev-plan/implement_20260405_203529.md` 기준 Phase 1~3 완료
  - shared summary / preview policy 정리
  - attach / project builder large-output UX polish
  - 문서 / smoke / regression sync
- `dev-plan/implement_20260405_205926.md` 기준 Phase 1~3 완료
  - soak archive / history manifest / release-candidate label baseline
...
| worker log visibility | 완료 | detached worker의 `stdout_log`, `stderr_log`, `stderr_tail`와 CLI `logs`, TUI `Logs` viewer 제공 |
| long-turn 상태 가시성 | 완료 | `attach` / `status` / `watch` / 앱에서 `executing-turn`, `settling`, `stale`, `heartbeat_age`, `turn_age` 표시 |
| generated preview UX | 완료 | `attach` / project builder 에서 prioritized file summary, `preview_headline`, `preview_excerpt`, `preview_selection`, `preview_trimmed`, hidden-file summary를 표시 |
| 사용자 quickstart 문서 | 완료 | `docs/USER_QUICKSTART.md`, `docs/TROUBLESHOOTING.md` 추가 |
| goal 기반 bootstrap | 완료 | `agent-team run "<goal>"`으로 goal 분석 기반 동적 역할 선택 + 자동 부팅 가능. `--roles`로 수동 오버라이드 지원 |
...
| repeated Codex soak | 실검증 완료 | `npm run soak:codex` + real `Codex CLI` 기준 `1/3/5 iteration` 통과, 단계별 `attach` snapshot과 `latest-summary.json`의 `verificationSummary` / `failurePatterns` 기록, `summary-*.json` / `history.json` 누적 보존 |
| release soak gate | 완료 | `docs/RELEASE_CHECKLIST.md`, `npm run soak:codex:check`, `permission/runtime/bridge` gate(`3/5/10 iteration`), `--history` / `--run-label` 지원 |
| multi-team picker / overview | 완료 | `tui` team picker가 attention-needed 팀을 위로 정렬하고 approvals/workers/tasks/attention reason을 함께 표시 |
| richer global dashboard / global ops surface | 완료 | team picker 상단에 Global Ops Overview를 추가해 cross-team attention/approval/stale/backlog 합계와 우선 팀을 바로 판독 가능 |
...
| 동적 역할 선택 | 완료 | goal 키워드 분석으로 10종 역할 풀에서 자동 선택, `--roles` 수동 오버라이드 지원 |
| 현재 테스트 수 | `233개 통과` | 최신 로컬 검증 기준 `0 fail` |
...
- `attach`는 이미 생성된 팀에 다시 붙어서 result state / teammate 상태 / worker lifecycle / task 집계 / 최근 activity / 생성 파일 summary / preview headline/excerpt / selection / trimmed metadata를 한 번에 요약합니다.
- `app`은 Codex 스타일 자연어 입력 대기 TUI를 띄우고, 첫 goal 입력 후 내부적으로 `run` bootstrap을 호출하는 제품형 진입점입니다.
- `run "<goal>"`은 goal 텍스트를 분석해 10종 역할 풀(`planner, search, frontend, backend, database, devops, testing, mobile, security, reviewer`)에서 필요한 역할만 자동 선택하고, background launch까지 수행하는 비대화형 실사용자 경로입니다. `--roles`로 수동 지정도 가능합니다.
...
- team picker / create
- attention-first multi-team overview list
- main dashboard
- tasks pane
- teammates pane
- activity feed
...
- `src/team-tui/app.tsx`
  ...
  - 팀 이름 없이 열면 team picker / overview부터 시작할 수 있다
  - picker는 `attention → running → pending → completed` 우선순위로 정렬되고, create mode에서도 `Esc`로 team list로 돌아갈 수 있다
- `src/team-tui/project-builder-app.tsx`
...
| [RELEASE_CHECKLIST.md](docs/RELEASE_CHECKLIST.md) | release gate / soak 판독 체크리스트 |
...
- real shoe-mall bootstrap 관측에서 `planner/search/backend/reviewer`가 먼저 완료되고, `frontend`만 raw task store 기준으로는 `pending=1` 상태에서 `executing-turn`을 유지하는 패턴을 확인했다. 현재 `attach` / `status` / dashboard / TUI task pane은 이를 공통 `effective task state`로 해석해 `in_progress`로 승격해서 보여준다.
- TUI boot smoke 통과
...
- file collision 방지용 scoped path / multi-area / overlap guardrail 표시 검증 통과
- team size 3~5 권장선 및 recent broadcast fan-out cost warning 표시 검증 통과
- repeated soak summary artifact(`soak-artifacts/latest-summary.json`)의 `verificationSummary` / `failurePatterns`와 `failure-*.json` structured failure taxonomy 기록 검증 통과
- repeated soak archive(`summary-*.json`) / `history.json` manifest / labeled checker(`--history --run-label`) 검증 통과

현재 최종 로컬 검증 결과:

- `npm run typecheck` 통과
- `npm test` 통과
- 총 `209 tests pass` (`2026-04-06` 재확인)
...
2. detached worker 로그는 이제 CLI `logs`와 TUI `Logs` tab으로 볼 수 있지만,  
여전히 전용 process control UI나 full log streaming transport는 없습니다.

3. permission rule UI parity는 실사용 가능한 수준입니다.  
CLI `permissions` / decision command와 TUI approval inbox는 `cmd/cwd/path/host` context와 matcher preset(`suggested/command/cwd/path/host`)을 공유합니다. 다만 richer policy management UI까지는 아직 아닙니다.

4. TUI와 project builder는 지금 충분히 실사용 가능하지만, richer global dashboard 확장 같은 전역 운영 surface에는 여지가 있습니다.
...
1. richer global dashboard / global ops surface
2. direct upstream parity / pane backend / remote transport는 실제 수요가 생길 때만 재평가하되, first candidate는 direct upstream parity로 둔다.
3. 더 긴 real-backend burn-in run은 release 후보가 생길 때 누적 실행
...
| 전체 인수인계 | `AGENT.md` |
| 제약/안정성 기준 확인 | `docs/TEAM_CONSTRAINTS.md` |
| 남은 backlog 우선순위 확인 | `docs/NEXT_BACKLOG.md` |
| 새 workstream 계획 생성 | `docs/DEV_PLAN_WORKFLOW.md`, `scripts/new_dev_plan.py`, `dev-plan/implement_20260405_201816.md`, `dev-plan/implement_20260405_205926.md` |
...
| soak / burn-in | `scripts/run-codex-repeated-soak.mjs`, `scripts/check-codex-repeated-soak.mjs`, `src/team-cli/soak/codex-repeated-soak.ts`, `docs/CODEX_REPEATED_SOAK.md`, `docs/RELEASE_CHECKLIST.md` |

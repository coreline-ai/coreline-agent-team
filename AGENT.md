...
- `attach` / `status` / TUI / 앱에서 long-turn runtime state 가시성(`executing-turn`, `settling`, `stale`) 추가
...
| repeated Codex soak | 실검증 완료 | `npm run soak:codex` + real `Codex CLI` 기준 `1/3/5 iteration` 통과 |
...
- `attach`는 이미 생성된 팀에 다시 붙어서 result state / teammate 상태 / task 집계 / 최근 activity / 생성 파일을 한 번에 요약합니다.
...
환경에 따라 `<repo-root>`는 상위 워크스페이스의 일부로 존재합니다.
...
2. generated files / preview 요약이 길 때 더 명확히 보이도록 UX 개선

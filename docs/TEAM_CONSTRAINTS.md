...
- 운영 가이드:
  ...
  - `run` 명령의 동적 역할 선택은 goal 키워드 분석 기반으로 10종 역할 풀에서 필요한 역할만 선택하므로,
    단순한 goal은 3개(planner + 1역할 + reviewer), 복잡한 goal은 7~8개까지 자동 확장된다.
  - `--roles` 플래그로 수동 제어도 가능하므로 비용 한계를 직접 관리할 수 있다.
